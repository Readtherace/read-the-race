...
8. **Once the result is known**, fill in `result`, `total_runners` (the
   ...
   overlay that carries what happened afterward: result, total_runners, starting_price,
...
`horses/results.csv`:
```
entry_id,date,course,framework_version,time_of_commitment,code,structure,category,distance,surface,going,selection,confidence,price_at_commitment,starting_price,result,principal_risk,material_change,process_compliance_grade,operator_disagreement,entry_file,total_runners
```
...
These columns contain the "Permanent record" fields each framework requires
(full list in the framework file) — the CSV is that record made queryable.
For horses, `total_runners` is additive factual result context so finishing
positions can be compared; it does not alter the framework or its predictions.
